...
struct csr_graph edge_list_to_csr(vector<std::pair<uint32_t,uint32_t>> const & v, uint32_t const max_vert){
    ...
    for (auto const& p : v){
    	...
    	//BOOST_LOG_TRIVIAL(info) << "vertex_from: " << p.first << " to:"<< p.second;
        if (vertex_from != prev_vertex_from) {
            //Fill in all rows that have no out edges
            for (uint32_t row = prev_vertex_from + 1;
                row <= vertex_from; ++row){
...
int main(int argc, char * argv[])
{
    try {
        ...
        fs::path p(file);
		//outdir.append(p.stem().c_str());
		//outdir2.append(p.stem().c_str());
        //BOOST_LOG_TRIVIAL(info) <<"outdir"<< outdir;
		outdir.append("csr");
        //BOOST_LOG_TRIVIAL(info) <<"outdir after append _csr"<< outdir;
		outdir2.append("csr");
        fs::path p2(outdir);
        fs::path p3(outdir2);
